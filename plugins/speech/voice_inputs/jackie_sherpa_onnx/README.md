...
- Raw ambient audio is not persisted or sent to Codex.
...
may reach the non-actuating Codex Brain only after that routing step.
...
protective-stop path from any voice-session state, including while Codex or TTS
is busy. The STOP spotter must emit only the safety-path hypothesis and cannot
open dictation. This software path does not replace a physical emergency stop
or target-side safety monitor.
