...
Jackie's voice path is deliberately separate from Codex. A local audio plugin
turns microphone audio into typed events; `WakeDictationController` enforces
the wake session; the existing interaction router handles deterministic
commands before optional open-ended text can reach Codex.
...
flowchart LR
    ...
    Context --> Codex["Optional Codex Brain"]
    Stop --> Runtime["Runtime protective stop"]
    Fixed --> Runtime
    Codex --> Gate["Proposal validation"]
    Gate --> Runtime
```
...
not import the tour package, call Codex, or access a target adapter.
...
Codex receives text and bounded Longship context only. It is not the
microphone, wake-word detector, ASR engine, TTS engine, memory authority, or
safety path.
...
1. wake false-accept and false-reject rates in the deployment languages and
   expected noise;
2. reserved STOP latency while ASR, TTS, Runtime, and Codex are busy;
